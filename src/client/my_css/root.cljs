...
(defui ^:once App
  ...
  (click-cb [this existing-colours grid-cell id selected?]
    (let [pick-colour-fn #(.pick-colour this existing-colours)]
      (if selected?
        (om/transact! grid-cell `[(graph/remove-line {:graph-ident [:trending-graph/by-id 10300] :intersect-id ~id}) :graph/trending-graph])
        (om/transact! grid-cell `[(graph/add-line {:graph-ident [:trending-graph/by-id 10300] :intersect-id ~id :colour ~(pick-colour-fn)}) :graph/trending-graph]))))
  (render [this]
